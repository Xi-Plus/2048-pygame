        ...
        for i in range(4):
            ...
            while j <= 2:
                if self.board[i][j] and self.board[i][j + 1] and self.board[i][j].val == self.board[i][j + 1].val:
                    ...
                    self.board[i][j + 1].nx = i
                    self.board[i][j + 1].ny = j
                    deleted_blocks.append(self.board[i][j + 1])
                    for k in range(j + 1, 3):
                        self.board[i][k] = self.board[i][k + 1]
                    self.board[i][3] = None
                    moved = True
                j += 1
        return moved, score, deleted_blocks
        ...
        for i in range(4):
            ...
            while j <= 2:
                if self.board[j][i] and self.board[j + 1][i] and self.board[j][i].val == self.board[j + 1][i].val:
                    ...
                    self.board[j + 1][i].nx = j
                    self.board[j + 1][i].ny = i
                    deleted_blocks.append(self.board[j + 1][i])
                    for k in range(j + 1, 3):
                        self.board[k][i] = self.board[k + 1][i]
                    self.board[3][i] = None
                    moved = True
